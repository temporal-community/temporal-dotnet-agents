...
/// Checks active session grants before parking the workflow for human approval;
...
internal sealed class ScopedApprovalInterceptor : IAgentToolInterceptor
{
    ...
    internal ScopedApprovalInterceptor(ApprovalScopesOptions opts) => ArgumentNullException.ThrowIfNull(opts);
        ...
        AgentToolContext context, CancellationToken cancellationToken)
    {
        ...
        if (context.StateBag is not null)
        {
            ...
            if (ApprovalScopeHelpers.TryMatchScope(
                ...
                context.ApprovalEvaluationTime,
